...
public class MenuOrderDAO implements MenuOrderDAO_Interface {
	...
	private static final String Insert_Stmt = "INSERT INTO MENU_ORDER (MENU_OD_ID,MENU_OD_STATUS,MENU_OD_START,MENU_OD_BOOK,CUST_ID,CHEF_ID,MENU_ID) VALUES ('MU'||TO_CHAR(CURRENT_DATE, 'YYYYMMDD')||'-'||LPAD(TO_CHAR(MENU_OD_ID_SEQ.NEXTVAL), 6, '0'),?,SYSDATE,?,?,?,?)";
	private static final String Update_Stmt = "UPDATE MENU_ORDER SET MENU_OD_STATUS = ?, MENU_OD_BOOK = ?, MENU_OD_END = ?, MENU_OD_RATE = ?, MENU_OD_MSG = ? ,CHEF_ID = ?, MENU_ID = ? WHERE MENU_OD_ID = ?";
	private static final String Update_Stmt_MenuOrderStatus = "UPDATE MENU_ORDER SET MENU_OD_STATUS = ? WHERE MENU_OD_ID = ?";
	private static final String Update_Stmt_MenuOrderStatus_MENU_OD_MSG = "UPDATE MENU_ORDER SET MENU_OD_STATUS = ? ,MENU_OD_END=? WHERE MENU_OD_ID = ?";
	private static final String Update_Stmt_MenuOrderRate = "UPDATE MENU_ORDER SET MENU_OD_RATE = ?, MENU_OD_MSG = ? WHERE MENU_OD_ID = ?";
	private static final String Delete_Stmt = "DELETE FROM MENU_ORDER WHERE MENU_OD_ID = ?";
	private static final String Get_One_Stmt = "SELECT * FROM MENU_ORDER WHERE MENU_OD_ID = ?";
	private static final String Get_One_Stmt_CustID = "SELECT * FROM MENU_ORDER WHERE CUST_ID = ?";
	private static final String Get_One_Stmt_ChefID = "SELECT * FROM MENU_ORDER WHERE CHEF_ID = ?";
	private static final String Get_All_Stmt = "SELECT * FROM MENU_ORDER";
	...
	public List<MenuOrderVO> findByChefID(String chef_ID) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(Get_One_Stmt_ChefID);
	...
	public void updateMenuOdStatus(String menu_od_ID, String menu_od_status) {
		...
		try {
			con = ds.getConnection();
			if (menu_od_status.equals("g4")) {

				pstmt = con.prepareStatement(Update_Stmt_MenuOrderStatus_MENU_OD_MSG);
